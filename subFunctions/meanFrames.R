...
initialMeanFrames <- function(frames, method= "Frechet") {
  ...
  sd1V3<-prcomp(t(frames[3,,]))$sdev[1]
  sd2V3<-prcomp(t(frames[3,,]))$sdev[2]
  ...
  if(sd1V3<1e-02 | sd2V3<1e-02){
    v3<-convertVec2unitVec(colMeans(t(frames[3,,])))
  }else if(method == "Frechet"){
    v3<-frechetMean(frames[3,,])
  }else if(method == "PNS"){
    sphereType<-kurtosisTestFunction(frames[3,,])
    v3<-pns(frames[3,,],sphere.type = sphereType)$PNS$mean
  }
  ...
  d<-acos(pmin(pmax((sum(meanV3*v3)) ,-1.0), 1.0))
